...
class UNETR_DP(UNETR):
    def __init__(self, in_channels, num_classes, props,
        ...
        super(UNETR_DP, self).__init__(in_channels,num_classes,props,img_size, feature_size, hidden_size, mlp_dim, num_heads,
                                       pos_embed, norm_name, conv_block, res_block, dropout_rate, default_return_skips)
...
# 본래의 영상 및 라벨의 사이즈를 줄이고 분할해서 (40*40)로 만든거라고 이해하면 된다.
    ...
    def forward(self, x, y=None, return_hard_tp_fp_fn=False):
        res = super(UNETR_DP, self).forward(x)  # regular Generic_UNet forward pass
