...
class DenseDownBlock_first(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, props, stride=None):
        ...
        # small version
        self.conv1 = props['conv_op'](in_planes, in_planes/2, kernel_size,
                                      padding=[(i - 1) // 2 for i in kernel_size],
                                      **kwargs_conv1)
        self.norm1 = props['norm_op'](in_planes/2, **props['norm_op_kwargs'])
        self.nonlin1 = props['nonlin'](**props['nonlin_kwargs'])

        self.conv2 = props['conv_op'](in_planes + (in_planes/2), in_planes, kernel_size,
                                      padding=[(i - 1) // 2 for i in kernel_size],
    ...
    def forward(self, x):
        ...
        out_1 = self.nonlin1(self.norm1(self.conv1(x)))  # 16
        residual_2 = out_1
        concat_1 = torch.cat((out_1, residual_1), dim=1)  # 32 + 16
...
class DenseDownBlock_2(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, props, stride=None):
        ...
        self.conv3 = props['conv_op']((in_planes * 2) + (in_planes/2), in_planes * 2, [1 for _ in kernel_size],
    ...
    def forward(self, x):
        ...
        concat_2 = torch.cat((out, residual_1), dim=1)  # 32 + 32
        concat_2 = torch.cat((concat_2,residual_2), dim = 1) # 32 + 32 + 16


        residual_out = self.nonlin3(self.norm3(self.conv3(concat_2))) # 64
        out = self.pool_op(residual_out)
...
class DenseUpBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, props, stride=None):
        ...
        self.conv0 = props['conv_op'](in_planes, aim_planes, [1 for _ in kernel_size], # 512 -> 256
                                      padding=[0 for i in kernel_size],
                                      **props['conv_op_kwargs'])
        self.norm0 = props['norm_op'](aim_planes, **props['norm_op_kwargs'])
        self.nonlin0 = props['nonlin'](**props['nonlin_kwargs'])

        self.conv1 = props['conv_op'](aim_planes, aim_planes/2, kernel_size, padding=[(i - 1) // 2 for i in kernel_size], # 256 -> 128
                                      **kwargs_conv1)
        self.norm1 = props['norm_op'](aim_planes/2, **props['norm_op_kwargs'])
        self.nonlin1 = props['nonlin'](**props['nonlin_kwargs'])

        self.conv2 = props['conv_op'](aim_planes + (aim_planes/2), aim_planes, kernel_size,  # 256 + 128 -> 256
                                      padding=[(i - 1) // 2 for i in kernel_size],
                                      **props['conv_op_kwargs'])
        self.norm2 = props['norm_op'](aim_planes/2, **props['norm_op_kwargs'])
        self.nonlin2 = props['nonlin'](**props['nonlin_kwargs'])

        self.conv3 = props['conv_op'](aim_planes * 2 + (aim_planes/2), aim_planes, [1 for _ in kernel_size], # 256 + 128 + 256 -> 256
                                      padding=[0 for i in kernel_size],
                                      **props['conv_op_kwargs'])
