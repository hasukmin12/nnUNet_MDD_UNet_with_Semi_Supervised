    ...
    def forward(self, x, y1=None,y2=None, y3=None, return_hard_tp_fp_fn=False):
        ...
        if self._deep_supervision and self.do_ds:
            if self.ureter and self.bladder and self.kidney:
                ...
                # for i in range(4):
                #     out1[i] = to_cuda(out1[i])
                #     out2[i] = to_cuda(out2[i])
                #     out3[i] = to_cuda(out3[i])

                # print("out1 : ",out1)
                # print("out1[0][0].shape : ", out1[0][0].shape)
                # print("out1[0][1].shape : ", out1[0][1].shape)
                # print("out1[0].shape : ",out1[0].shape)
                # print("out1[0][0].shape : ",out1[0][0].shape)

                # print("[i(j) for i, j in zip(list(self.upscale_logits_ops)[::-1] : ", [i(j) for i, j in zip(list(self.upscale_logits_ops)[::-1])])
                # print("seg_outputs1[:-1][::-1].shape : ", seg_outputs1[:-1][::-1].shape)
                # print("seg_outputs1[:-1][::-1] : ",seg_outputs1[:-1][::-1])

                # print("zip(list(self.upscale_logits_ops)[::-1] : ",zip(list(self.upscale_logits_ops)[::-1]))
            ...
            ce_losses1 = [self.ce_loss(to_cuda_3(out1[0]), y1[0]).unsqueeze(0)]
            # tp : True Positive
            # fp : False Positive
            # fn : False Negative
            tps1 = []
            fps1 = []
            fns1 = []
            res_softmax1 = softmax_helper(to_cuda_3(out1[0]))
            tp1, fp1, fn1, _ = get_tp_fp_fn_tn(res_softmax1, y1[0])
            tps1.append(tp1)
            fps1.append(fp1)
            fns1.append(fn1)
            for i in range(1, len(y1)):
                ce_losses1.append(self.ce_loss(to_cuda_3(out1[i]), y1[i]).unsqueeze(0))
                res_softmax1 = softmax_helper(to_cuda_3(out1[i]))
            ...
            ce_losses2 = [self.ce_loss(to_cuda_1(out2[0]), y2[0]).unsqueeze(0)]
            # tp : True Positive
            # fp : False Positive
            # fn : False Negative
            tps2 = []
            fps2 = []
            fns2 = []
            res_softmax2 = softmax_helper(to_cuda_1(out2[0]))
            tp2, fp2, fn2, _ = get_tp_fp_fn_tn(to_cuda_1(res_softmax2), y2[0])
            tps2.append(tp2)
            fps2.append(fp2)
            fns2.append(fn2)
            for i in range(1, len(y2)):
                ce_losses2.append(self.ce_loss(to_cuda_1(out2[i]), y2[i]).unsqueeze(0))
                res_softmax2 = softmax_helper(to_cuda_1(out2[i]))
            ...
            ce_losses3 = [self.ce_loss(to_cuda_2(out3[0]), y3[0]).unsqueeze(0)]
            # tp : True Positive
            # fp : False Positive
            # fn : False Negative
            tps3 = []
            fps3 = []
            fns3 = []
            res_softmax3 = softmax_helper(to_cuda_2(out3[0]))
            tp3, fp3, fn3, _ = get_tp_fp_fn_tn(res_softmax3, y3[0])
            tps3.append(tp3)
            fps3.append(fp3)
            fns3.append(fn3)
            for i in range(1, len(y3)):
                ce_losses3.append(self.ce_loss(to_cuda_2(out3[i]), y3[i]).unsqueeze(0))
                res_softmax3 = softmax_helper(to_cuda_2(out3[i]))
        ...
        # 여기론 갈일 없는듯?
            ...
            ce_loss1 = self.ce_loss(seg_outputs1[-1], y1).unsqueeze(0)
            # tp fp and fn need the output to be softmax
            res_softmax1 = softmax_helper(seg_outputs1[-1])
            tp1, fp1, fn1, _ = get_tp_fp_fn_tn(res_softmax1, y1)
            ret1 = ce_loss1, tp1, fp1, fn1

            ce_loss2 = self.ce_loss(seg_outputs2[-1], y2).unsqueeze(0)
            # tp fp and fn need the output to be softmax
            res_softmax2 = softmax_helper(seg_outputs2[-1])
            tp2, fp2, fn2, _ = get_tp_fp_fn_tn(res_softmax2, y2)
            ret2 = ce_loss2, tp2, fp2, fn2

            ce_loss3 = self.ce_loss(seg_outputs3[-1], y3).unsqueeze(0)
            # tp fp and fn need the output to be softmax
            res_softmax3 = softmax_helper(seg_outputs3[-1])
        ...
        if return_hard_tp_fp_fn:
            ...
            if self._deep_supervision and self.do_ds:
                ...
                target1 = y1
                output1 = seg_outputs1[-1]
                target2 = y2
                output2 = seg_outputs2[-1]
                target3 = y3
                output3 = seg_outputs3[-1]
