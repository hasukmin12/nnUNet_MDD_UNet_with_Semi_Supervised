...
class nnUNetTrainerV2_for_DTC(nnUNetTrainer):
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if self.fp16:
            with autocast():
                output, output_tanh = self.network(data)
                del data
                ...
                if target[0].max() == 0:
                    ...
                    # 새로 제안하는 loss_sdf (그냥 tanh과 GT를 바로 비교)
                    # loss_sdf = self.loss([dis_to_mask], [target[0]]) -1
                    # total_consistency_loss = (consistency_loss + loss_sdf) / 2
                    #
                    # beta = 0.2
                    ...
                    # # visualize
                    # # dis_to_mask = dis_to_mask.detach().cpu().numpy()
                    # #
                    # output_tanh = output_tanh.detach().cpu().numpy()
                    # # outputs_soft = outputs_soft.detach().cpu().numpy()
                    # gt_dis = gt_dis.detach().cpu().numpy()
                    # a = l
                    # output_n = output[0].detach().cpu().numpy()
                    # target_n = target[0].detach().cpu().numpy()
                    # target_soft = target_soft.detach().cpu().numpy()
        ...
        #     # else:
            ...
            #
            #
            #     loss_sdf = self.loss([gt_dis], [dis_to_mask]) -1
            #
            #     beta = 0.3
            #     l = beta * (loss_sdf + consistency_loss) + (DC_l * (1-beta))
