...
from nnunet.network_architecture.SDDense_UNet import get_default_network_config
from nnunet.network_architecture.SDDense_UNet_DP import SDDenseUNet_DP

from nnunet.network_architecture.initialization import InitWeights_He
from nnunet.training.network_training.nnUNetTrainer import nnUNetTrainer
from nnunet.training.network_training.nnUNetTrainerV2 import nnUNetTrainerV2
from nnunet.training.network_training.nnUNet_variants.data_augmentation.nnUNetTrainerV2_DA3_for_DTC import nnUNetTrainerV2_DA3_for_DTC

from nnunet.utilities.nd_softmax import softmax_helper

class HasTrainer_DP_SDDense(nnUNetTrainerV2_DA3_for_DTC): #nnUNetTrainerV2):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, num_gpus=4, distribute_batch_size=False, fp16=False):
        super(HasTrainer_DP_SDDense, self).__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage,
    ...
    def initialize_network(self):
        ...
        self.network = SDDenseUNet_DP(self.num_input_channels, self.base_num_features, blocks_per_stage_encoder, 2,
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: np.ndarray, do_mirroring: bool = False,
        ...
        ds = self.network.decoder.deep_supervision
        self.network.decoder.deep_supervision = False
        ret = nnUNetTrainer.predict_preprocessed_data_return_seg_and_softmax(self, data, do_mirroring=False, #do_mirroring,
    ...
    def predict_preprocessed_data_return_attention(self, data: np.ndarray, do_mirroring: bool = False,
                                                         mirror_axes: Tuple[int] = None,
                                                         use_sliding_window: bool = True, step_size: float = 0.5,
                                                         use_gaussian: bool = True,
                                                         pad_border_mode: str = 'constant',
        ...
        ds = self.network.decoder.deep_supervision
        self.network.decoder.deep_supervision = False
        ret = nnUNetTrainer.predict_preprocessed_data_return_attention(self, data, do_mirroring=False, # do_mirroring,
                                                                             mirror_axes=mirror_axes,
                                                                             use_sliding_window=use_sliding_window,
                                                                             step_size=step_size,
                                                                             use_gaussian=use_gaussian,
...
nnUNetTrainerV2_ResencUNet_copy1 = HasTrainer_DP_SDDense
nnUNetTrainerV2_ResencUNet_copy2 = HasTrainer_DP_SDDense
nnUNetTrainerV2_ResencUNet_copy3 = HasTrainer_DP_SDDense
nnUNetTrainerV2_ResencUNet_copy4 = HasTrainer_DP_SDDense
